...
#region Constructors
        ...
        public Database(IMapper defaultMapper = null)
        {
            ...
            //ConnectionStringSettings entry = ConfigurationManager.ConnectionStrings[0];
        ...
        public Database(string connectionStringName, IMapper defaultMapper = null)
        {
            ...
            _connectionString = configurationBuilder.Build().GetSection(connectionStringName).Value;
        ...
        //    ConnectionStringSettings entry = ConfigurationManager.ConnectionStrings[connectionStringName];
        ...
        public Database(IDatabaseBuildConfiguration configuration)
        {
            ...
            IBuildConfigurationSettings settings = (IBuildConfigurationSettings) configuration;
...
#region Command Management
        ...
        private void AddParam(IDbCommand cmd, object value, PropertyInfo pi)
        {
            ...
            if (pi != null)
            {
                IMapper mapper = Mappers.GetMapper(pi.DeclaringType, _defaultMapper);
        ...
        private void SetParameterProperties(IDbDataParameter p, object value, PropertyInfo pi)
        {
            ...
            else
            {
                ...
                {
                    string asValue = (value as AnsiString).Value;
                    if (asValue == null)
                    {
                        p.Size = 0;
                        p.Value = DBNull.Value;
...
#region Exception Reporting and Logging
        ...
        public virtual bool OnException(Exception x)
        {
            ...
            ExceptionEventArgs args = new ExceptionEventArgs(x);
        ...
        {
            return ExecuteInternal(CommandType.Text, sql, args);
        }
        ...
        {
            return ExecuteScalarInternal<T>(CommandType.Text, sql, args);
        }
        ...
        {
            return Fetch<T>(page, itemsPerPage, string.Empty);
        }
        ...
        {
            return Page<T>(page, itemsPerPage, string.Empty);
        }
        ...
        {
            return SkipTake<T>(skip, take, string.Empty);
        }
        ...
        {
            TableInfo poco = PocoData.ForType(typeof(T), _defaultMapper).TableInfo;

            if (sqlCondition.TrimStart().StartsWith("where", StringComparison.OrdinalIgnoreCase))
            {
                const int StartIndex = 5;
                sqlCondition = sqlCondition.TrimStart()[StartIndex..];
            }
        ...
        {
            PocoData poco = PocoData.ForType(typeof(T), _defaultMapper);
...
#region operation: Insert
        ...
        public object Insert(string tableName, object poco)
        {
            ...
            return ExecuteInsert(tableName, pd?.TableInfo.PrimaryKey, pd != null && pd.TableInfo.AutoIncrement, poco);
        }
        ...
        public object Insert(string tableName, string primaryKeyName, object poco)
        {
            ...
            Type t = poco.GetType();
            PocoData pd = PocoData.ForType(poco.GetType(), _defaultMapper);
            bool autoIncrement = pd == null || pd.TableInfo.AutoIncrement || t.Name.Contains("AnonymousType") &&
        ...
        private object ExecuteInsert(string tableName, string primaryKeyName, bool autoIncrement, object poco)
        {
            try
            {
                ...
                try
                {
                    using IDbCommand cmd = CreateCommand(_sharedConnection, string.Empty);
                    PocoData pd = PocoData.ForObject(poco, primaryKeyName, _defaultMapper);
        ...
        private void PreExecuteUpdate(string tableName, string primaryKeyName, object poco, object primaryKeyValue, IEnumerable<string> columns, IDbCommand cmd)
        {
            ...
            if (primaryKeyName != null)
            {
                pkpi = pd.Columns.TryGetValue(primaryKeyName, out PocoColumn col) ? col.PropertyInfo : new { Id = primaryKeyValue }.GetType().GetProperties()[0];
            }
        ...
        public int Delete(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            ...
            string sql = $"DELETE FROM {_provider.EscapeTableName(tableName)} WHERE {_provider.EscapeSqlIdentifier(primaryKeyName)}=@0";
        ...
        public int Delete<T>(object pocoOrPrimaryKey)
        {
            ...
            {
                PropertyInfo pi = pocoOrPrimaryKey.GetType().GetProperty(pd.TableInfo.PrimaryKey);
...
#region operation: IsNew
        ...
        protected virtual bool IsNew(string primaryKeyName, PocoData pd, object poco)
        {
            ...
            Type type = pk != null ? pk.GetType() : pi.PropertyType;
...
#region operation: Save
        ...
        {
            return Query<TRet>(new[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4), typeof(T5) }, cb, sql.SQL, sql.Arguments);
        }
        ...
        {
            return Query<T1>(new[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4), typeof(T5) }, null, sql.SQL, sql.Arguments);
        }
        ...
        public IEnumerable<TRet> Query<TRet>(Type[] types, object cb, string sql, params object[] args)
        {
            ...
            try
            {
                ...
                Func<IDataReader, object, TRet> factory = MultiPocoFactory.GetFactory<TRet>(types, _sharedConnection.ConnectionString, sql, r, _defaultMapper);
                if (cb == null)
        ...
        public IGridReader QueryMultiple(string sql, params object[] args)
        {
            ...
            IDbCommand cmd = CreateCommand(_sharedConnection, sql, args);

            try
            {
                IDataReader reader = ExecuteReaderHelper(cmd);
        ...
        {
            return QueryProc<T>(storedProcedureName, args).ToList();
        }
