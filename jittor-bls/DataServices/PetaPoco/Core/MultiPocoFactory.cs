...
namespace PetaPoco.Internal
{
    internal class MultiPocoFactory
    {
        ...
        public static object GetAutoMapper(Type[] types)
        {
            ...
            return AutoMappers.Get(key, () =>
            {
                ...
                for (int i = 1; i < types.Length; i++)
                {
                    ...
                    {
                        // Find the property
                        System.Reflection.PropertyInfo[] candidates = (from p in types[j].GetProperties() where p.PropertyType == types[i] select p).ToArray();
                        if (!candidates.Any())
                            continue;
        ...
        private static Delegate FindSplitPoint(Type typeThis, Type typeNext, string connectionString, string sql, IDataReader r, ref int pos, IMapper defaultMapper)
        {
            ...
            PocoData pdThis = PocoData.ForType(typeThis, defaultMapper);
            PocoData pdNext = PocoData.ForType(typeNext, defaultMapper);
        ...
        {
            DynamicMethod m = new DynamicMethod("petapoco_multipoco_factory", typeof(TRet), new[] { typeof(MultiPocoFactory), typeof(IDataReader), typeof(object) },
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                Delegate del = FindSplitPoint(types[i], i + 1 < types.Length ? types[i + 1] : null, connectionString, sql, r, ref pos, defaultMapper);
                dels.Add(del);
                ...
                // Call Invoke
                System.Reflection.MethodInfo tDelInvoke = del.GetType().GetMethod("Invoke");
        ...
        {
            Tuple<Type, ArrayKey<Type>, string, string> key = Tuple.Create(typeof(TRet), new ArrayKey<Type>(types), connectionString, sql);
