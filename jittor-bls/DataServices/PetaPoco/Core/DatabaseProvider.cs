...
namespace PetaPoco.Core
{
    ...
    public abstract class DatabaseProvider : IProvider
    {
        ...
        {
            Type ft = null;
            foreach (string assemblyName in assemblyQualifiedNames)
            {
        ...
        {
            foreach (string initialString in customProviders.Keys)
                if (name.IndexOf(initialString, StringComparison.InvariantCultureIgnoreCase) == 0 && customProviders.TryGetValue(initialString, out IProvider provider))
                    return provider;
        ...
        internal static IProvider Resolve(Type type, bool allowDefault, string connectionString)
        {
            ...
            IProvider custom = GetCustomProvider(typeName);
            if (custom != null)
                return custom;
        ...
        internal static IProvider Resolve(string providerName, bool allowDefault, string connectionString)
        {
            ...
            IProvider custom = GetCustomProvider(providerName);
            if (custom != null)
                return custom;
