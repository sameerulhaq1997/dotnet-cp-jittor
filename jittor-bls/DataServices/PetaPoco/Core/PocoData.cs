...
namespace PetaPoco.Core
{
    public class PocoData
    {
        ...
        private readonly Cache<Tuple<string, string, int, int>, Delegate> PocoFactories = new Cache<Tuple<string, string, int, int>, Delegate>();
        public Type Type;
        ...
        public static PocoData ForObject(object obj, string primaryKeyName, IMapper defaultMapper)
        {
            ...
            if (t == typeof(System.Dynamic.ExpandoObject))
            {
                ...
                foreach (string col in (obj as IDictionary<string, object>).Keys)
                {
        ...
        {
            // Check cache
            Tuple<string, string, int, int> key = Tuple.Create<string, string, int, int>(sql, connectionString, firstColumn, countColumns);
            ...
            return PocoFactories.Get(key, () =>
            {
                ...
                if (Type == typeof(object))
                {
                    ...
                    {
                        Type srcType = reader.GetFieldType(i);
                ...
                else
                {
                    ...
                    for (int i = firstColumn; i < firstColumn + countColumns; i++)
                    {
                        ...
                        if (!Columns.TryGetValue(reader.GetName(i), out PocoColumn pc))
                            continue;
                    ...
                    MethodInfo fnOnLoaded = RecurseInheritedTypes<MethodInfo>(Type,
        ...
        private static Func<object, object> GetConverter(IMapper mapper, PocoColumn pc, Type srcType, Type dstType)
        {
            ...
            {
                Type backingDstType = Enum.GetUnderlyingType(dstType);
                if (underlyingDstType != null)
                {
