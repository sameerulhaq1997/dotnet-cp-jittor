...
#region PocoFromIDataReader
        ...
        private IEnumerable<T> SinglePocoFromIDataReader<T>(int index)
        {
            ...
            try
            {
                ...
                {
                    Func<IDataReader, T> factory = pd.GetFactory(_command.CommandText, _command.Connection.ConnectionString, 0, _reader.FieldCount, _reader, _defaultMapper) as Func<IDataReader, T>;
        ...
        private IEnumerable<TRet> MultiPocoFromIDataReader<TRet>(int index, Type[] types, object cb)
        {
            ...
            try
            {
                IDbCommand cmd = _command;
                IDataReader r = _reader;

                Func<IDataReader, object, TRet> factory = MultiPocoFactory.GetFactory<TRet>(types, cmd.Connection.ConnectionString, cmd.CommandText, r, _defaultMapper);
                if (cb == null)
