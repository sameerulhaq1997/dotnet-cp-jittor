...
namespace PetaPoco
{
    ...
    {
        private static readonly ConcurrentDictionary<object, Lazy<IMapper>> _mappers = new ConcurrentDictionary<object, Lazy<IMapper>>();
        ...
        public static bool Register(Assembly assembly, IMapper mapper)
        {
            return RegisterInternal(assembly, mapper);
        }
        ...
        /// <param name="mapper">The IMapper implementation</param>
        public static bool Register(Type type, IMapper mapper)
        {
            return RegisterInternal(type, mapper);
        }
        ...
        public static bool Revoke(Assembly assembly)
        {
            return RevokeInternal(assembly);
        }
        ...
        public static bool Revoke(Type type)
        {
            return RevokeInternal(type);
        }
        ...
        {
            bool ret = false;
            KeyValuePair<object, Lazy<IMapper>> m = _mappers.FirstOrDefault(v => v.Value.Value == mapper);
            object key = m.Equals(default(KeyValuePair<object, Lazy<IMapper>>)) ? null : m.Key;
            if (key != null)
            {
                ret = _mappers.TryRemove(key, out Lazy<IMapper> _);
                if (ret) FlushCaches();
            }
            return ret;
        }
        ...
        {
            bool ret = _mappers.TryAdd(typeOrAssembly, new Lazy<IMapper>(() => mapper));
            if (ret) FlushCaches();
            return ret;
        }
        ...
        {
            bool ret = _mappers.TryRemove(typeOrAssembly, out Lazy<IMapper> _);
            if (ret) FlushCaches();
            return ret;
        }
