...
namespace Jittor.App.Services
{
    public static class ExtensionService
    {
        ...
        public static Sql BuildWhereClause(this Sql sql, PageFilterModel filter, int index)
        {
            ...
            return sql.Append($" {(index == 0 ? "" : filter.Operation)} ({filter.Field} {filter.Operator} {(string.IsNullOrEmpty(filter.TryConvertType) ? "@0" : $"TRY_CONVERT({filter.TryConvertType}, @0)")} ) ", filter.Value ?? "");
        }
